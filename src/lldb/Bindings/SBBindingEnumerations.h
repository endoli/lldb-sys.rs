...
/// This doesn't work with Rust's bindgen either.
#ifndef BINDGEN
...
FLAGS_ENUM(LaunchFlags){
    ...
    eLaunchFlagStopAtEntry =
        (1u
         << 2), ///< Stop at the program entry point instead of auto-continuing
                ///< when launching or attaching at entry point
    ...
    eLaunchFlagDontSetExitStatus =
        (1u << 8), ///< If you are going to hand the process off (e.g. to
                   ///< debugserver) set this flag so lldb & the handee don't
...
FLAGS_ENUM(SymbolContextItem){
    eSymbolContextTarget = (1u
                            << 0), ///< Set when \a target is requested from a
                                   ///< query, or was located in query results
    eSymbolContextModule = (1u
                            << 1), ///< Set when \a module is requested from a
                                   ///< query, or was located in query results
    eSymbolContextCompUnit =
        (1u << 2), ///< Set when \a comp_unit is requested from a query, or was
                   ///< located in query results
    eSymbolContextFunction =
        (1u << 3), ///< Set when \a function is requested from a query, or was
                   ///< located in query results
    eSymbolContextBlock =
        (1u << 4), ///< Set when the deepest \a block is requested from a query,
                   ///< or was located in query results
    eSymbolContextLineEntry =
        (1u << 5), ///< Set when \a line_entry is requested from a query, or was
                   ///< located in query results
    eSymbolContextSymbol = (1u
                            << 6), ///< Set when \a symbol is requested from a
                                   ///< query, or was located in query results
    eSymbolContextEverything =
        ((eSymbolContextSymbol << 1) -
         1u), ///< Indicates to try and lookup everything up during a routine
              ///< symbol context query.
    eSymbolContextVariable =
        (1u << 7) ///< Set when \a global or static variable is requested from a
                  ///< query, or was located in query results.
                  ///< eSymbolContextVariable is potentially expensive to lookup
                  ///< so it isn't included in eSymbolContextEverything which
                  ///< stops it from being used during frame PC lookups and many
                  ///< other potential address to symbol context lookups.
};
...
FLAGS_ENUM(BreakpointEventType){
    ...
    eBreakpointEventTypeLocationsAdded =
        (1u << 3), // Locations added doesn't get sent when the breakpoint is
                   // created
...
FLAGS_ENUM(FunctionNameType){
    ...
    eFunctionNameTypeFull =
        (1u << 2), // The function name.
                   // For C this is the same as just the name of the function
                   // For C++ this is the mangled or demangled version of the
                   // mangled name. For ObjC this is the full function signature
                   // with the + or
                   // - and the square brackets and the class and selector
    eFunctionNameTypeBase =
        (1u << 3), // The function name only, no namespaces or arguments and no
                   // class methods or selectors will be searched.
...
// to frame B the following cases arise: 1) When frame A pushes frame B (or a
// frame that ends up pushing B) A is Older than B. 2) When frame A pushed frame
